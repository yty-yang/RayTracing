            ...
            for m in (0.125, 0.375, 0.625, 0.875):
                for n in (0.125, 0.375, 0.625, 0.875):
                    for k in range(0, 100):
                        rnumx = random.uniform(-0.01, 0.01)
                        rnumy = random.uniform(-0.01, 0.01)

                        current = [j + 0.5 + m, i + 0.5 + n, 1]  # the node ray goes through
...
def create_file():
    ...
    f = open("cube_16&depth.ppm", "w", encoding="ASCII")
    f.write("P3\n")
    f.write("150 100\n")
    f.write("255\n")
