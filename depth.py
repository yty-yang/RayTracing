...
# 这一版直接抖动了摄像机，实际应该从每个屏幕像素中心旁边随机取点向焦点发射射线
    ...
    for i in range(1, 101):
        ...
        for j in range(1, 151):
            ...
            for k in range(0, 100):
                ...
                rnumz = random.uniform(-0.02, 0.02)
                camera = [rnumx, rnumy, rnumz]

                current = [j + 0.5, i + 0.5, 1]  # the node ray goes through
                l = (current[0] ** 2 + current[1] ** 2 + 1) ** 0.5
                focuspoint = [current[0] * focus / l, current[1] * focus / l, current[2] * focus / l]
                direction = [focuspoint[0] - rnumx, focuspoint[1] - rnumy, focuspoint[2] - rnumz]
...
def create_file():
    ...
    f = open("cube_depth.ppm", "w", encoding="ASCII")
    f.write("P3\n")
    f.write("150 100\n")
    f.write("255\n")
